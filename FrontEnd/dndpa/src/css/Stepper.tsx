...
export default function Stepper({
    ...
    const stepLabels = stepsArray.map((child) => {
        if (React.isValidElement(child)) {
            return (child.props as StepProps).label ?? String(index + 1);
        }
        return '';
    });
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators, label}: StepIndicatorProps) {
